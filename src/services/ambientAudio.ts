...
class AmbientSoundEngine {
  ...
  // Tactile Page Turn Sound: Procedural paper flutter & leaf rustle
  public playPageTurnSound(direction: 'forward' | 'backward' = 'forward') {
    try {
      const ctx = this.getContext();
      const bufferSize = Math.floor(ctx.sampleRate * 0.16); // 160ms
      const buffer = ctx.createBuffer(1, bufferSize, ctx.sampleRate);
      const data = buffer.getChannelData(0);

      // Generate textured pinkish noise for organic paper friction
      let b0 = 0, b1 = 0, b2 = 0;
      for (let i = 0; i < bufferSize; i++) {
        const white = Math.random() * 2 - 1;
      ...
      // Bandpass filter to isolate crisp paper flutter
      const filter = ctx.createBiquadFilter();
      filter.type = 'bandpass';
      filter.Q.value = 1.6;
  ...
  // Tactile Bookmark Ribbon Latch
  public playBookmarkSound() {
    try {
      const ctx = this.getContext();
      const now = ctx.currentTime;

      // Dual micro-click + soft thud
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.type = 'sine';
  ...
  // Tactile Highlighter Felt Swish
